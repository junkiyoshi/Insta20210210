...
void ofApp::setBoxToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float height, float width, float depth) {
	...
	for (int i = index; i < face_target.getNumVertices(); i++) {
		...
		auto rotation_y = glm::rotate(glm::mat4(), ofMap(ofNoise(face_target.getVertex(i).y * 0.0008 + ofGetFrameNum() * 0.01), 0, 1, PI * -2.5, PI * 2.5), glm::vec3(0, 1, 0));
		auto rotation_x = glm::rotate(glm::mat4(), ofMap(ofNoise(face_target.getVertex(i).x * 0.0008 + ofGetFrameNum() * 0.01), 0, 1, PI * -2.5, PI * 2.5), glm::vec3(1, 0, 0));
